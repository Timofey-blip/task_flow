...
Фукция can_transition? - Для проверки возможности выполнения перехода по событию.

Функция available_events -  Нужна, чтобы получить все доступные события для текущего состояния(Возвращает массив символов событий).

Функця state - Для получения текущего состояние (имя). Ниже(с параметром) - применяется для тестов.

Функция transitions_history - Используется для получения истории всех переходов.

Функция reset - Сбрасывает машину состояний до начального состояния.
